...
void SimplePattern::trigger_on_for_step(int step) {
    ...
    if (this->output!=nullptr) {
        this->output->receive_event(1,0,0,this->get_velocity());
        this->output->process();
        note_held = true;
    }
}
void SimplePattern::trigger_off_for_step(int step) {
    ...
    if (this->output!=nullptr) {
        this->output->receive_event(0,1,0,this->get_velocity());
        this->output->process();
        note_held = false;
    }
};
...
#ifdef ENABLE_SCREEN
    void BasePattern::create_menu_items(Menu *menu, int pattern_index, BaseSequencer *, int combine_patterns) {
